...
public class Uninvite extends SubCommand {
    ...
    public String getUsage() {
        return "/team uninvite <player>";
    }

    @Override
    public String getDescription() {
        return "Uninvite a player from your team. (only works for leaders)";
    }
    ...
    public void perform(Player player, String[] args) {
        ...
        if (args.length == 1){
            player.sendMessage(ColorUtil.CC("&c/team uninvite <player>"));
            return;
        }
        if (args.length > 1){
            ...
            if (found == null){
                player.sendMessage(prefix + ColorUtil.CC(lang.getString("CANT_UNINVITE")));
                return;
            }
            if (team == null){
                player.sendMessage(prefix + ColorUtil.CC(lang.getString("CANT_UNINVITE")));
            ...
            player.sendMessage(prefix + ColorUtil.CC(lang.getString("UNINVITED")
                    .replace("%player%", args[1])));
            teamManager.uninvitePlayer(team, found);
        }
    }
}
